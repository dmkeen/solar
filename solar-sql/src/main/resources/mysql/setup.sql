...
CREATE TABLE IF NOT EXISTS current_power (id SERIAL, epoch_timestamp BIGINT, generation NUMERIC(6,2), consumption NUMERIC(6,2), primary key (id), index idx_epoch_timestamp (epoch_timestamp))
CREATE TABLE IF NOT EXISTS generation_forecast (id SERIAL, pv_estimate NUMERIC(6,4), pv_estimate10 NUMERIC(6,4), pv_estimate90 NUMERIC(6,4), period_end_epoch BIGINT, period_length_seconds INT, primary key (id), unique index idx_period_end_epoch (period_end_epoch))
...
CREATE TABLE IF NOT EXISTS measurement_upload (last_uploaded_epoch BIGINT)
